...
export const AIVolumeCoach: React.FC<AIVolumeCoachProps> = ({
  ...
  const [activeCategory, setActiveCategory] = useState<'all' | 'ignored' | RecommendationCategory>('all');
  const [appliedIds, setAppliedIds] = useState<Record<string, boolean>>({});
  const [dismissedIds, setDismissedIds] = useState<Record<string, boolean>>({});
  const [isAllDismissed, setIsAllDismissed] = useState<boolean>(false);
  ...
  const handleDismiss = (recId: string) => {
    setDismissedIds((prev) => ({ ...prev, [recId]: true }));
  };

  const handleRestore = (recId: string) => {
    setDismissedIds((prev) => {
      const copy = { ...prev };
      delete copy[recId];
      return copy;
    });
    if (isAllDismissed) setIsAllDismissed(false);
  };

  const handleToggleDismissAll = () => {
    if (isAllDismissed) {
  ...
  // Raccomandazioni non ignorate
  const activeRecommendations = analysis.recommendations.filter(
    (r) => !dismissedIds[r.id] && !isAllDismissed
  );

  // Raccomandazioni ignorate
  const ignoredRecommendations = analysis.recommendations.filter(
    (r) => dismissedIds[r.id] || isAllDismissed
  );
  ...
  const filteredList =
    activeCategory === 'ignored'
      ...
      : (analysis.byCategory[activeCategory] || []).filter(
          (r) => !dismissedIds[r.id] && !isAllDismissed
        );
  ...
  const effectiveScore = isAllDismissed || activeRecommendations.length === 0 ? 100 : analysis.overallScore;
  const effectiveScoreLabel =
    isAllDismissed || activeRecommendations.length === 0
      ? 'Volume Approvato dal Coach'
      : analysis.scoreLabel;
  ...
  return (
    ...
    >
      {/* ─── 1. HEADER COMPATTO CON SCORE, PULSANTE IGNORA TUTTO & KPI ─── */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">
              Diagnosi biomeccanica, anteprima modifiche settimanali & ottimizzazioni
            </p>
          </div>
        </div>

        {/* Azione Mantieni così + Score Sintetico */}
        <div className="flex items-center gap-2.5 flex-wrap self-start sm:self-auto">
          <button
            type="button"
            onClick={handleToggleDismissAll}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer border shadow-sm ${
              isAllDismissed || activeRecommendations.length === 0
                ? 'bg-slate-900 text-amber-400 border-amber-500/40 hover:bg-slate-850'
          ...
          >
            {isAllDismissed || activeRecommendations.length === 0 ? (
              <>
                ...
                <span>Ripristina Suggerimenti IA</span>
              </>
            ) : (
              <>
                <CheckCheck className="w-3.5 h-3.5 text-emerald-400" />
                <span>Mantieni così / Ignora criticità</span>
              </>
            )}
          </button>

          <div className="flex items-center gap-2 bg-slate-950 p-1.5 px-3 rounded-xl border border-slate-800 shrink-0">
            <span className="text-[10px] uppercase font-bold text-slate-400">Score:</span>
            <span className="text-xs font-black text-slate-200">{effectiveScoreLabel}</span>
            <span className={`font-mono text-xs font-black px-2 py-0.5 rounded-lg border ml-1 ${
              effectiveScore >= 90
      ...
      {/* ─── 2. SELETTORE A PILLOLE PER LE CATEGORIE ─── */}
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pt-0.5">
        ...
        >
          ...
          <span>Attivi ({activeRecommendations.length})</span>
        </button>
        ...
        {analysis.criticalCount > 0 && (
          ...
          >
            ...
            <span>Criticità ({activeRecommendations.filter((r) => r.category === 'critical').length})</span>
          </button>
        )}
        ...
        {analysis.optimizationCount > 0 && (
          ...
          >
            ...
            <span>Ottimizzazioni ({activeRecommendations.filter((r) => r.category === 'optimization').length})</span>
          </button>
        )}
        ...
        {ignoredRecommendations.length > 0 && (
          ...
          >
            ...
            <span>Ignorati ({ignoredRecommendations.length})</span>
          </button>
        )}
      </div>

      {/* ─── 3. BANNER "VOLUME APPROVATO DAL COACH" SE TUTTE LE CRITICITÀ SONO IGNORATE ─── */}
      {(isAllDismissed || (activeCategory === 'all' && activeRecommendations.length === 0 && ignoredRecommendations.length > 0)) ? (
        <div className="p-6 sm:p-8 rounded-2xl bg-emerald-950/20 border border-emerald-500/30 text-center space-y-3 animate-in fade-in">
          <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 mx-auto flex items-center justify-center shadow-lg">
            <ShieldCheck className="w-6 h-6" />
          </div>
          <div className="space-y-1">
            <h4 className="text-sm sm:text-base font-black text-white uppercase tracking-wider">
              Volume & Struttura Approvati dal Coach
            </h4>
            <p className="text-xs text-slate-300 max-w-lg mx-auto leading-relaxed">
              Hai scelto di mantenere il programma con i volumi e la ripartizione attuali. Nessuna modifica automatica verrà forzata.
            </p>
          </div>
          <div className="pt-2 flex items-center justify-center gap-3 flex-wrap">
            <button
              type="button"
              onClick={handleToggleDismissAll}
              className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-amber-400 hover:text-amber-300 border border-slate-700 text-xs font-bold transition-all cursor-pointer inline-flex items-center gap-1.5 shadow-sm"
            >
              <RotateCcw className="w-3.5 h-3.5" />
              <span>Mostra di nuovo suggerimenti IA</span>
            </button>
          </div>
        </div>
      ) : (
        /* ─── LISTA DELLE CARD CONSIGLIO CON DETTAGLIO MODIFICHE ─── */
        <div className="space-y-3">
          {displayedList.length === 0 ? (
            ...
            displayedList.map((rec) => {
              ...
              const isDismissed = dismissedIds[rec.id] || isAllDismissed;
              ...
              return (
                ...
                >
                  {/* Riga 1: Header Sintetico (Priorità + Distretto + Diagnosi + Tasti Azione) */}
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
                      {isApplied ? (
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-850 hover:bg-slate-800 text-amber-400 text-xs font-bold border border-slate-700 transition-all cursor-pointer"
                          title="Ripristina nei suggerimenti attivi"
                        >
                        ...
                        <>
                          ...
                          <button
                            ...
                            className="p-1.5 px-2 rounded-lg text-slate-400 hover:text-white bg-slate-900 hover:bg-slate-850 border border-slate-800 text-xs font-bold transition cursor-pointer flex items-center gap-1"
                            title="Ignora questo suggerimento e mantieni il volume attuale"
                          >
                            <EyeOff className="w-3.5 h-3.5 text-slate-400" />
                            <span className="text-[11px] hidden sm:inline">Ignora</span>
                          </button>
                        </>
                      )}
      ...
      {activeCategory === 'all' && filteredList.length > 4 && !showAllItems && !isAllDismissed && activeRecommendations.length > 0 && (
