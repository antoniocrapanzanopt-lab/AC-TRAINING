...
export const AssignWorkoutModal: React.FC<AssignWorkoutModalProps> = ({ workout, onClose }) => {
  ...
  const { assignWorkoutToAthlete, allAssignedWorkouts } = useWorkouts();
  ...
  // Identifica gli atleti a cui questa scheda è già assegnata
  ...
  const currentlyAssignedAthletes = activeAthletes.filter(a => assignedAthleteIds.has(a.id));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-[var(--color-panel)] border border-[var(--color-panel-border)] rounded-2xl shadow-2xl flex flex-col">
        ...
        <div className="p-6 space-y-5">
          <div className="bg-slate-800/30 border border-slate-700/50 p-4 rounded-xl space-y-2">
            <p className="text-xs text-slate-400 uppercase font-bold tracking-wider mb-1">Stai assegnando:</p>
            <p className="text-white font-bold text-base">{workout.title}</p>
            {workout.description && <p className="text-xs text-slate-400">{workout.description}</p>}
          </div>

          {currentlyAssignedAthletes.length > 0 && (
            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl space-y-1.5">
              <p className="text-xs font-bold text-amber-400">Atleti che stanno già usando questo programma:</p>
              <div className="flex flex-wrap gap-1.5">
                {currentlyAssignedAthletes.map(ath => (
                  <span key={ath.id} className="text-xs font-bold px-2.5 py-1 bg-amber-500/20 text-amber-300 rounded-lg flex items-center gap-1 border border-amber-500/30">
                ...
                {activeAthletes.map(athlete => {
                  ...
                  return (
                    <option key={athlete.id} value={athlete.id}>
                      {athlete.firstName} {athlete.lastName} {isAssigned ? '(Già Assegnato)' : ''}
                    </option>
                  );
                })}
              </select>
