...
import { X, Send, User, Search, Users, Check } from 'lucide-react';
...
export const AssignWorkoutModal: React.FC<AssignWorkoutModalProps> = ({ workout, onClose }) => {
  ...
  const handleAssign = async () => {
    ...
    setIsAssigning(true);
    try {
      const { success, error } = await assignWorkoutToAthletes(selectedAthleteIds, workout.id);
        ...
        <div className="flex items-center justify-between p-5 border-b border-[var(--color-panel-border)] bg-slate-950/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Scegli a chi inviare il programma (selezione singola o multipla)</p>
            </div>
          </div>
          <button
            onClick={onClose}
