...
const typeConfig: Record<CalendarEventType, { label: string; color: string; bgBadge: string; icon: React.FC<{ className?: string }> }> = {
  ...
  appointment: { label: 'Appuntamento', color: 'text-blue-400 border-blue-400/30', bgBadge: 'bg-blue-500/20 text-blue-300', icon: Clock },
  ...
  program_delivery: { label: 'Consegna Programma', color: 'text-cyan-400 border-cyan-400/30', bgBadge: 'bg-cyan-500/20 text-cyan-300', icon: FileText },
  ...
  google_calendar: { label: 'Google Calendar', color: 'text-sky-400 border-sky-400/30', bgBadge: 'bg-sky-500/20 text-sky-300', icon: Globe },
...
export const CalendarPage: React.FC = () => {
  ...
  // Auto scroll a 07:00 / 08:00 quando si passa alla vista Settimana o Giorno
  useEffect(() => {
  ...
  // Clic su uno slot orario nella griglia h.24
  ...
  return (
    <div className="space-y-6 pb-12">
      {/* Header Titolo & Sincronizzazione */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Calendario Appuntamenti</h1>
          <p className="text-sm text-slate-400 mt-1">
            Gestisci sessioni, check-in e appuntamenti con vista oraria h.24 e sincronizzazione Google.
          </p>
        </div>
      ...
      {/* Control Bar: Navigazione, Selettore Vista e Filtro */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 p-4 rounded-2xl bg-[var(--color-panel)] border border-[var(--color-panel-border)] shadow-xl">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 shrink-0">
            <button onClick={handlePrev} className="p-2 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors">
            ...
            <button onClick={handleNext} className="p-2 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
          <h2 className="text-base sm:text-lg font-black text-white capitalize">{headerTitle}</h2>
        </div>

        {/* Selettore Modalità Vista & Filtro Eventi */}
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center bg-slate-900 border border-slate-800 rounded-xl p-1 text-xs font-bold">
            ...
            >
              <Columns className="w-3.5 h-3.5" /> Settimana (24h)
            </button>
            <button
            ...
            >
              <CalendarIcon className="w-3.5 h-3.5" /> Giorno (24h)
            </button>
            <button
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="appointment">🔵 Appuntamenti / Sessioni</option>
      ...
      {/* ======================================================================== */}
      {/* VISTA 1: SETTIMANA (GRIGLIA h.24)                                        */}
      {/* ======================================================================== */}
      {viewMode === 'week' && (
        <div className="rounded-2xl bg-[var(--color-panel)] border border-[var(--color-panel-border)] shadow-xl overflow-hidden flex flex-col">
          {/* Header Giorni della Settimana */}
          <div className="grid grid-cols-8 border-b border-slate-800 bg-slate-950/60 sticky top-0 z-20">
            {/* Slot Angolo Ora */}
            <div className="p-3 border-r border-slate-800 text-center text-xs font-bold text-slate-500 uppercase flex items-center justify-center">
              Ora (h.24)
            </div>
            ...
            {weekDays.map((d, i) => (
              <div
                ...
                className={`p-3 text-center border-r border-slate-800 last:border-r-0 cursor-pointer transition-colors ${
                  d.isToday ? 'bg-amber-950/20' : 'hover:bg-slate-900/50'
                }`}
              >
                <span className="block text-[11px] font-bold uppercase text-slate-400">{d.dayName}</span>
                <span
                  className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-black mt-1 ${
                    d.isToday
                      ? 'bg-[var(--color-primary)] text-black shadow-md'
                      : 'text-white'
                  }`}
          ...
          {/* Griglia Oraria 24h con Scroll Verticale */}
          <div ref={scrollRef} className="max-h-[650px] overflow-y-auto relative divide-y divide-slate-800/60">
            {HOURS.map(hour => {
              const hourLabel = `${String(hour).padStart(2, '0')}:00`;

              return (
                <div key={hour} className="grid grid-cols-8 min-h-[52px] group">
                  {/* Etichetta Oraria (Es. 08:00) */}
                  <div className="p-2 border-r border-slate-800/80 text-right pr-3 text-xs font-semibold text-slate-500 bg-slate-950/40 select-none flex items-start justify-end">
                    {hourLabel}
                  </div>

                  {/* 7 Celle per i giorni della settimana a quest'ora */}
                  {weekDays.map((day, dayIdx) => {
                    const dayEvts = (eventsByDate[day.dateStr] || []).filter(e => {
                      if (!e.startTime) return hour === 0;
                    ...
                    return (
                      <div
                        ...
                        className="p-1 border-r border-slate-800/60 last:border-r-0 relative hover:bg-amber-500/5 transition-colors cursor-pointer min-h-[52px]"
                      >
                        {dayEvts.map(evt => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-1.5 rounded-lg border text-xs mb-1 transition-all hover:scale-[1.02] shadow-md flex flex-col justify-between ${
                                evt.isSystemGenerated
                                  ? 'bg-slate-900/90 border-slate-700'
                                  : 'bg-slate-900 border-[var(--color-primary)]/40'
                              }`}
                            >
                              <div className="flex items-center gap-1 min-w-0">
                                <IconComp className={`w-3.5 h-3.5 shrink-0 ${cfg.color.split(' ')[0]}`} />
                                <span className="font-bold text-white truncate text-[11px]">{evt.title}</span>
                              </div>
                              <div className="flex items-center justify-between text-[10px] text-slate-400 mt-1">
                                <span>{evt.startTime ? evt.startTime : 'Tutto il giorno'}</span>
                                {evt.athleteName && (
                                  <span className="truncate max-w-[70px] text-amber-300 font-semibold">{evt.athleteName}</span>
                                )}
                              </div>
                            </div>
                          );
      ...
      {/* ======================================================================== */}
      {/* VISTA 2: GIORNO (GRIGLIA h.24 DETTAGLIATA)                              */}
      {/* ======================================================================== */}
      {viewMode === 'day' && (
        <div className="rounded-2xl bg-[var(--color-panel)] border border-[var(--color-panel-border)] shadow-xl overflow-hidden flex flex-col">
          <div className="p-4 border-b border-slate-800 bg-slate-950/60 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Vista Giornaliera Dettagliata</span>
              <h3 className="text-lg font-black text-white capitalize">{headerTitle}</h3>
            </div>
            <button
              onClick={() => handleSlotClick(currentDate.toISOString().slice(0, 10), new Date().getHours())}
              className="px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-700 text-xs font-bold text-slate-200 hover:text-white transition-colors"
            >
              + Aggiungi Ora Corrente
            </button>
          </div>
            ...
            {HOURS.map(hour => {
              ...
              return (
                <div key={hour} className="flex min-h-[65px] group">
                  {/* Orario Left */}
                  <div className="w-20 p-3 border-r border-slate-800/80 text-right text-xs font-bold text-slate-400 bg-slate-950/40 select-none shrink-0 flex items-start justify-end">
                    {hourLabel}
                  </div>
                  ...
                  <div
                    ...
                    className="flex-1 p-2 hover:bg-amber-500/5 transition-colors cursor-pointer flex flex-wrap gap-2 items-start"
                  >
                    {dayEvts.length === 0 ? (
                      <span className="text-[11px] text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity p-1">
                        + Fai clic per aggiungere un evento alle {hourLabel}
                      </span>
                    ) : (
                      dayEvts.map(evt => {
                        const cfg = typeConfig[evt.type] || typeConfig.custom;
                        ...
                        return (
                          <div
                            ...
                            className="p-3 rounded-xl border bg-slate-900 border-[var(--color-primary)]/30 hover:border-[var(--color-primary)] text-xs transition-all shadow-lg flex-1 min-w-[280px] max-w-md"
                          >
                            <div className="flex items-center justify-between gap-2 mb-1.5">
                              <div className="flex items-center gap-2">
                                <span className={`p-1.5 rounded-lg border ${cfg.color}`}>
                                  <IconComp className="w-4 h-4" />
                                </span>
                                <div>
                                  <h4 className="font-black text-white text-sm">{evt.title}</h4>
                                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${cfg.bgBadge}`}>
                                    {cfg.label}
                                  </span>
                                </div>
                              </div>
              ...
              {calendarGrid.map((cell, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-[75px] sm:min-h-[95px] p-2 rounded-xl border flex flex-col justify-between text-left transition-all relative ${
                      isSelected
                  ...
                  >
                    <div className="flex justify-between items-center w-full">
                      <span className={`text-xs font-bold ${cell.isToday ? 'text-[var(--color-primary)]' : 'text-slate-300'}`}>
                        {cell.dayNumber}
                      </span>
                      {dayEvts.length > 0 && (
                        <span className="w-4 h-4 rounded-full bg-[var(--color-primary)] text-black font-black text-[9px] flex items-center justify-center">
                          {dayEvts.length}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1 w-full mt-1 overflow-hidden">
                      ...
                      {dayEvts.length > 3 && (
                        <span className="text-[9px] font-bold text-slate-400">+{dayEvts.length - 3} altri</span>
                      )}
                    </div>
                  </button>
                );
          ...
          <div className="p-5 rounded-2xl bg-[var(--color-panel)] border border-[var(--color-panel-border)] shadow-xl flex flex-col space-y-4">
            <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-[var(--color-primary)]/10 text-[var(--color-primary)] border border-[var(--color-primary)]/30 hover:bg-[var(--color-primary)]/20 transition-colors"
              ...
              {(eventsByDate[selectedDateStr] || []).length === 0 ? (
                ...
                (eventsByDate[selectedDateStr] || []).map(evt => {
                  ...
                  return (
                    <div
                      key={evt.id}
                      className="p-3.5 rounded-xl border bg-slate-900 border-slate-800 space-y-2"
                    >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
                          ...
                          {evt.athleteName && <span>• Atleta: <strong className="text-amber-400">{evt.athleteName}</strong></span>}
      ...
      {deleteModal.open && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[var(--color-panel)] border border-[var(--color-panel-border)] rounded-2xl shadow-2xl p-6">
            ...
            <p className="text-sm text-slate-400 mb-6">Sei sicuro di voler eliminare questo evento dal calendario?</p>
