...
import { X, Calendar as CalendarIcon, AlertTriangle, Clock, User, MapPin, Tag, ExternalLink } from 'lucide-react';
import { CalendarEvent, CalendarEventFormData, CalendarEventType } from '../../types';
import { useAthletes } from '../../context/AthletesContext';
import { generateGoogleCalendarWebUrl } from '../../services/googleCalendarService';
...
export const CalendarModal: React.FC<CalendarModalProps> = ({
  ...
  const currentWebUrl = generateGoogleCalendarWebUrl({
    title: title || 'Nuovo Evento Athlete Manager',
    description,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      ...
      <div className="relative w-full max-w-2xl bg-[var(--color-panel)] border border-[var(--color-panel-border)] rounded-2xl shadow-2xl flex flex-col my-auto max-h-[90vh] z-10">
        {/* Header */}
        ...
        {/* Body */}
        <form id="calendar-event-form" onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1">
          {errors.length > 0 && (
          ...
          {/* Quick Google Calendar Web Link Generator */}
          <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2 text-xs text-slate-300 font-semibold">
              <span className="w-2 h-2 rounded-full bg-emerald-400"></span>
              Sincronizzazione Web 1-Click:
            </div>
            <a
              href={currentWebUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-sky-500/10 border border-sky-500/30 text-sky-300 text-xs font-bold hover:bg-sky-500/20 transition-all"
            >
              <ExternalLink className="w-3.5 h-3.5" /> Aggiungi a Google Calendar (Web)
            </a>
          </div>
        </form>
