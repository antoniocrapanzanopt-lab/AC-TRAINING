...
BEGIN
   ...
   RETURN EXISTS (
      SELECT 1 FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email'))
   );
END;
...
CREATE POLICY "athlete_own_profile" ON athletes FOR SELECT TO authenticated USING (LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email')));
CREATE POLICY "athlete_update_own_profile" ON athletes FOR UPDATE TO authenticated USING (LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email'))) WITH CHECK (LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email')));
...
USING (athlete_id IN (SELECT id FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email'))))
WITH CHECK (athlete_id IN (SELECT id FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email'))));
...
USING (id IN (SELECT workout_id FROM athlete_workouts WHERE athlete_id IN (SELECT id FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email')))));
...
USING (workout_id IN (SELECT workout_id FROM athlete_workouts WHERE athlete_id IN (SELECT id FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email')))));
...
USING (session_id IN (SELECT id FROM workout_sessions WHERE athlete_id IN (SELECT id FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email')))))
WITH CHECK (session_id IN (SELECT id FROM workout_sessions WHERE athlete_id IN (SELECT id FROM athletes WHERE LOWER(TRIM(email)) = LOWER(TRIM(auth.jwt()->>'email')))));
